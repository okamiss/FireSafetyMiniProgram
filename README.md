...
已完成工程基础、身份权限、消防报修、消防培训、消息通知和数据管理里程碑：
...
- 管理后台看板展示报修总量、状态分布、培训完成率和培训通过率。
- 支持报修数据、培训记录及答题明细 `.xlsx` 导出，对用户输入执行 Excel 公式注入防护。
- 记录管理员登录、权限审批和报修状态变更等基础操作日志，并支持后台筛选查询。

安全加固、验收回归和上线交付仍按一次性交付计划继续推进。

## 看板、导出与日志说明

- 培训完成率按“至少提交过一次答卷人数 / 已分配人数”统计，通过率按“已通过人数 / 已提交人数”统计。
- 培训导出文件包含“培训记录”和“答题明细”两个工作表；所有业务时间按 `Asia/Shanghai` 输出。
- 标准操作日志只覆盖管理员登录、权限审批和报修状态变更，不包含增强版的敏感数据查看、导出审计或完整多维看板。
